    ...
    <main className="flex-1 bg-[#fffcf7] text-stone-900">
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1fr_24rem] lg:px-8 lg:py-20">
        <div className="space-y-14">
          ...
          {course.type === "live" && <section><h2 className="text-3xl font-bold">{t("liveSchedule")}</h2>{course.live_sessions?.length ? <ul className="mt-6 grid gap-3 sm:grid-cols-2">{course.live_sessions.map((session) => <li key={session.id} className="rounded-2xl border border-stone-200 bg-white p-5 font-semibold">{new Intl.DateTimeFormat(locale, { dateStyle: "full", timeStyle: "short" }).format(new Date(session.starts_at))}</li>)}</ul> : <p className="mt-5 text-stone-600">{t("schedulePending")}</p>}</section>}
