...
export default async function LocaleLayout({
  ...
  const [settings, tNav] = await Promise.all([getSiteSettings(), getTranslations("Navigation")]);
  ...
  return (
    ...
    >
      <body className="flex min-h-full flex-col">
        <a
          href="#main-content"
          className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[100] focus:rounded-lg focus:bg-amber-800 focus:px-4 focus:py-2 focus:text-sm focus:font-bold focus:text-white focus:outline-2 focus:outline-offset-2 focus:outline-amber-950"
        >
          {tNav("skipToContent")}
        </a>
