...
export function EnrollmentForm({ course, upiId, upiQrUrl }: { course: Course; upiId: string | null; upiQrUrl: string | null }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setEnrollment(await api.enrollments.create({ course_id: course.id, client: contact, guest: !token }, token));
    } catch (error) {
  ...
  if (enrollment) {
    return (
      <section className="rounded-[2rem] border border-stone-200 bg-white p-7 shadow-sm" aria-live="polite">
        ...
        {!isDemo && upiId ? <div className="mt-6 grid gap-5 rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:grid-cols-[1fr_auto] sm:items-center"><div><h3 className="font-bold text-amber-950">{t("paymentTitle")}</h3><p className="mt-2 text-sm text-amber-900/75">{t("paymentDescription", { reference: enrollment.reference_number })}</p><p className="mt-3 rounded-xl bg-white px-4 py-3 font-bold text-amber-900">{upiId}</p></div>{upiQrUrl && <>{/* Tenant-controlled URL from Settings; supported hosts are not known at build time. */}<img src={upiQrUrl} alt={t("qrAlt")} className="size-36 rounded-xl bg-white object-contain p-2" /></>}</div> : !isDemo ? <p className="mt-6 rounded-2xl bg-blue-50 p-4 text-sm text-blue-900">{t("paymentUnavailable")}</p> : null}
